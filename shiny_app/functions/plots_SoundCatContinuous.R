plots_SoundCateg <- function(plottype_SCCONT,
                             datelim_SCCONT,
                             stage_filter_SCCONT,
                             animal_filter_SCCONT,
                             exp_SCCONT,
                             f_options_SCCONT) {
  ...
  if(missing(datelim_SCCONT)){
    datelim_SCCONT = c(max(TRAINING$date)-9,max(TRAINING$date))
  }
  
  
  if (f_options_SCCONT == "All animals") {
    TRAINING <- TRAINING %>%
      dplyr::filter(
        choice_direction == "right_trials",
        stage %in% stage_filter_SCCONT,
        date >= datelim_SCCONT[1], date <= datelim_SCCONT[2],
  ...
  if (f_options_SCCONT == "Experimenter") {
    TRAINING <- TRAINING %>%
      dplyr::filter(
        choice_direction == "right_trials",
        stage %in% stage_filter_SCCONT,
  ...
  if (f_options_SCCONT == "Individual animals") {
    TRAINING <- TRAINING %>%
      dplyr::filter(
        choice_direction == "right_trials",
        stage %in% stage_filter_SCCONT,
        date >= datelim_SCCONT[1], date <= datelim_SCCONT[2],
        protocol == "@SoundCatContinuous",
        animal_id == animal_filter_SCCONT
      )
  ...
  if (plottype_SCCONT == "CP duration") {
    cp_plot <- ggplot(
      data = TRAINING,
      mapping = aes(
        x = date,
      ...
      scale_x_date(
        ...
        date_labels = "%b %d",
        minor_breaks = "1 day",
        limits = c(as.Date(datelim_SCCONT[1]), as.Date(datelim_SCCONT[2]))
      ) +
      theme(
  ...
  if (plottype_SCCONT == "Choice direction") {
  ...
  if (plottype_SCCONT == "No. done trials") {
    trial_plot <- ggplot(
      data = TRAINING,
      mapping = aes(
        x = date,
  ...
  if (plottype_SCCONT == "No. completed trials") {
    trial_plot <- ggplot(
      data = TRAINING,
      mapping = aes(
        x = date,
  ...
  if (plottype_SCCONT == "No. correct trials") {
    trial_plot <- ggplot(
      data = TRAINING,
      mapping = aes(
        x = date,
  ...
  if (plottype_SCCONT == "Correct ratio") {
    trial_plot <- ggplot(
      data = TRAINING,
      mapping = aes(
        x = date,
      ...
      annotate("text",x = datelim_SCCONT[1], y = 0.51, label = "Chance level", col = "gray") + 
      labs(col = eval(parse(text = "col_lab_name")))
  ...
  if (plottype_SCCONT == "Stage tracking") {
    stage_plot <- ggplot(
      data = TRAINING,
  ...
  if (plottype_SCCONT == "Missing data") {
