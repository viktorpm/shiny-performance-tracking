plots_AltStat <- function(plottype_AS,
                            datelim_AS,
                            stage_filter_AS,
                            animal_filter_AS,
                            exp_AS,
  ...
  if(missing(datelim_AS)){
    datelim_AS = c(max(TRAINING$date)-9,max(TRAINING$date))
  }
  
  
  
  if (f_options_AS == "All animals") {
    TRAINING <- TRAINING %>%
      dplyr::filter(
        choice_direction == "right_trials",
        stage %in% stage_filter_AS,
        date >= datelim_AS[1], date <= datelim_AS[2],
  ...
  if (f_options_AS == "Individual animals") {
    TRAINING <- TRAINING %>%
      dplyr::filter(
        choice_direction == "right_trials",
        stage %in% stage_filter_AS,
  ...
  if (plottype_AS == "CP duration") {
    cp_plot <- ggplot(
      ...
      scale_x_date(
        ...
        date_labels = "%b %d",
        minor_breaks = "1 day",
        limits = c(as.Date(datelim_AS[1]), as.Date(datelim_AS[2]))
      ) +
      theme(
  ...
  if (plottype_AS == "Choice direction") {
  ...
  if (plottype_AS == "No. completed trials") {
    trial_plot <- ggplot(
      data = TRAINING,
      mapping = aes(
        x = date,
  ...
  if (plottype_AS == "No. correct trials") {
    trial_plot <- ggplot(
      data = TRAINING,
      mapping = aes(
        x = date,
  ...
  if (plottype_AS == "Correct ratio") {
    trial_plot <- ggplot(
      ...
      annotate("text", x = datelim_AS[1], y = 0.51, label = "Chance level", col = "gray") +
      labs(col = eval(parse(text = "col_lab_name")))
