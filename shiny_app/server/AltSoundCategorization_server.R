      ...
      output$perform_ASC <- DT::renderDataTable(
        TRAINING %>%
          dplyr::filter(
            ...
            protocol == "@AltSoundCategorization",
...
### @AltSoundCategorization generate report ----

output$report_ASC <- downloadHandler(
  filename = "weekly_report_ASC.html",
  content = function(file) {
    tempReport <- file.path(tempdir(), "weekly_report_ASC.Rmd") %>%
      normalizePath()
    file.copy(from = "weekly_report_ASC.Rmd", to = tempReport, overwrite = T)
    library(rmarkdown)
    params_ASC <- list(
      exp_ASC = input$exp_select_ASC,
