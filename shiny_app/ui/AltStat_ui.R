tabPanel(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(
        inputId = "exp_select_AS",
        label = "Select experimenter",
        choices = TRAINING$experimenter %>% unique() %>% as.vector()
      ),
      
      downloadButton("report_AS", "Generate report"),
      
      
      selectInput(
        inputId = "animal_select_AS",
    ...
    mainPanel(
      width = 9,
      plotOutput(outputId = "plot_AS", height = 1000),
      dataTableOutput(outputId = "perform_AS")
    )
  )
)