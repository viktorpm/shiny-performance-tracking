...
time_ranges <- left_join(water, sessions) %>% 
  ...
  dplyr::mutate(
     Training = difftime(
      ...
      units = "min") %>% as.numeric() %>% replace_na(""),
    Watering = difftime(
...
time_ranges %>%
  ...
  formattable::formattable(
    align = c("c", "c", "r", "r", "r", "r", "r", "c","c" ),
    list(
      `Watering` = color_bar(color =  "#AAD6E6"),
      `Training` = color_bar(color = "pink")
  ...
  formatter("span",
    style = x ~ style(
     ...
     x ~ icontext(ifelse(
       ...
       yes = "arrow-up",
       no = ifelse(c(NA, diff(as.numeric(x))) < 0, "arrow-down", "minus") ), x)
  )
