...
Rats table is updated in the db. It's enough to pull that

```{r}
# rats <- dplyr::tbl(
#   src = akrami_db,
#   sql("SELECT * FROM rats_original")
# ) %>% 
#   as_tibble()
#   # mutate_all(list(~na_if(.,"")))
# 
# 
# rats_pyrat <- read_csv(file.path("D:","_R_WD","git_projects","r_codes_rat_wm","data","animals.csv")) %>%
#   as_tibble()
# 
# rats_new <- left_join(rats, rats_pyrat)
# rats_new
...
record_weights <- left_join(
  ...
  mutate(current_weight = NA)

record_weights

# record_weights %>% datatable()

```


Shiny app: text input field in DataTable: 

https://stackoverflow.com/questions/48264943/text-input-column-in-a-table-in-shiny
https://stackoverflow.com/questions/62346988/write-data-table-with-text-inputs-to-csv-in-shiny

reproducible shiny app example to save csv
```{r}
library(shiny)
library(DT)
library(dplyr)
...
to check active (in training) animals in order to select the water restricted ones 
