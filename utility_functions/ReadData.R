...
#' @param trialData Process trial data (TRUE) or not (FALSE).
  ...
  if (trialData == F) {
    ...
    if ("SessionData" %in% names(rat_data)) {
      ...
      TRAINING <- ReadBpodData(
        rds_file = rds_file, data_source = data_source, rat_data = rat_data
      )
      return(TRAINING)
    } else if ("saved" %in% names(rat_data)) {
      data_source <- "bcontrol"
      TRAINING <- ReadBcontrolData(
        rds_file = rds_file, rat_data = rat_data, data_source = data_source
      )
      return(TRAINING)
    }
  }
  ...
  if (trialData == T) {
    ...
    if (csv_exists) {
      read_TRIAL <- suppressMessages(
        suppressWarnings(read_csv(file.path(path, "TrialByTrial.csv")))
      )
      file_processed_test <- str_detect(
        read_TRIAL$file, regex(paste0(rds_file))
      )
    } else {
