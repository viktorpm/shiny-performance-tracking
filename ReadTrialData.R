...
ReadTrialData <- function(rds_file, rat_data, data_source) {
  ...
  trial_by_trial = list(
    ...
    date = rat_data$saved[, , ]$SavingSection.SaveTime %>% as.character() %>% substr(1, 11),
    ...
    hit = ifelse(
      ...
      yes = "empty_field_in_mat_file",
      no = get(paste(section_name, ".hit.history", sep = ""), rat_data$saved[, , ]) %>% as.vector()),
    choice = ifelse(
      rat_data$saved[, , ]$SideSection.previous.sides %>% intToUtf8(multiple = T) %>% is_empty(),
      yes = "empty_field_in_mat_file",
      no = rat_data$saved[, , ]$SideSection.previous.sides %>% intToUtf8(multiple = T)
    ),
    trial_per_session = seq(from = 1, 
                            to = get(paste(section_name, ".hit.history", sep = ""), rat_data$saved[, , ]) %>% length())
  )
